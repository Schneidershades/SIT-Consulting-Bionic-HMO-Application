...
<div class="col-sm-9">
	<h4 class="page-title">{{$hcp->hcp_name}}</h4>
	<ol class="breadcrumb">
		<li class="breadcrumb-item"><a href="javaScript:void();">{{auth()->user()->userable->hmo_name}}</a></li>
		<li class="breadcrumb-item active" aria-current="page"> {{$hcp->hcp_name}}</li>
	</ol>
</div>
<div class="col-sm-3">
	...
	<div class="card">
		<div class="card-body">
			...
			<div class="form-group row">
				...
				<div class="col-sm-6">
					@if($hcp->hasAccount($hcp->id) == 'yes')
					<span class="btn btn-gradient-royal m-1">Account is created</span>
					@else
					<button class="btn btn-gradient-royal m-1" data-toggle="modal" data-target="#royalmodal">Create Account for {{$hcp->hcp_code}} - {{$hcp->hcp_name}}</button>

					<div class="modal fade" id="royalmodal">
						<div class="modal-dialog">
							<div class="modal-content gradient-royal border-0">
								<div class="modal-header">
									<h5 class="modal-title text-white"><i class="fa fa-star"></i>Create Account for {{$hcp->hcp_code}} - {{$hcp->hcp_name}}</h5>
								...
								<div class="modal-body text-white">
									<form class="color-form" method="POST" action="{{route('create.hcps.account')}}">
										...
										<div class="form-row">
											...
											<input name="userable_id" type="hidden" value="{{$hcp->id}}">
				...
				<div class="form-row">
					...
					<div class="col-md-3">
						...
						<h5>{{$hcp->hcp_state ? $hcp->hcp_state : 'Not Available'}}</h5>
					</div>
					...
					<div class="col-md-3">
						...
						<h5>{{$hcp->hcp_bank_account_number ? $hcp->hcp_bank_account_number : 'Not Available'}}</h5>
					</div>
				</div><br><br>	
